            ...
            if oneSuite.get('name').upper()==self.targetSuite.upper():
                ...
                if oneSuite.get('repositoryType').upper()=="FILENAMELIST":
                    ...
                    for oneFile in iterfileList:
                        self.listTargetTestdataFile.append(iterfileList + "\" + oneFile.text)
...
if __name__ == "__main__":
    filename=os.getcwd()+"/Automation-Configuration.xml"
    print(filename)
    LoadConfigureXmlIns=LoadConfigureXml(filename,"Suite_test1")
